...
class Player:
    def __init__(self, x, y):
        ...
        self.weapon = Weapon(owner=self, damage=3, range=100, bullet_speed=5, fire_rate=0.5)
        ...
        self.bases = [Base(0, 0, color=arcade.color.GOLD)]
        self.bases[0].main_base = self.bases[0]
    ...
    def try_spawn_base(self, wells):
        if self.scrap >= BASE_COST and time.time() - self.last_base_spawn_time >= self.base_spawn_cooldown and self.distance_from_base < self.maximum_capable_distance:
            ...
            for well in wells:
                if get_distance(new_base.center_x, well.center_x, new_base.center_y, well.center_y) < well.radius:
                    new_base.is_collecting = True
                    new_base.assigned_well = well
                    break
    ...
    def handle_input(self, keys, wells):
        if keys.get(arcade.key.SPACE, False):
            self.try_spawn_base(wells)
        self.update_movement(keys)

    def update(self, enemies, bullets, keys, wells):
        self.distance_from_base = get_distance(
            self.center_x, self.bases[-1].center_x,
            self.center_y, self.bases[-1].center_y
        ) / KILOMETER if self.bases else math.hypot(self.center_x, self.center_y) / KILOMETER
        ...
        for base in self.bases:
            if base.is_collecting:
                ...
                self.active_flows.append(
                    ...
                    x1=base.center_x,
                    y1=base.center_y,
                    x2=base.main_base.center_x,
                    y2=base.main_base.center_y,
                    width=2,
                    color=arcade.color.LIGHT_BLUE))
            else:
                self.active_collecting_bases.discard(base)

    def draw(self, enemies):
        points = [(0, 0)] + [(b.center_x, b.center_y) for b in self.bases] + [(self.center_x, self.center_y)]
        ...
        if self.distance_from_base <= self.maximum_capable_distance:
            ...
            arcade.draw_line(self.center_x, self.center_y, self.bases[-1].center_x, self.bases[-1].center_y, (0, 0, 0, 50), 1)
